...
class ObjectReplicator(Daemon):
    ...
    def __init__(self, conf):
        ...
        self.node_errors = dict([(dev['id'], {}) for dev in self.object_ring.devs])
        self.last_replication_time = (-1, -1)
        #

    ## M
    def error_limited(self, node_id):
        """
        Check if the node is currently error limited.
        :param node: dictionary of node to check
        :returns: True if error limited, False otherwise
        """
        node = self.node_errors.get(node_id)
        now = time.time()
        if 'errors' not in node:
            return False
        if 'last_error' in node and node['last_error'] < now - self.error_suppression_interval:
            if 'errors' in node:
                del node['errors']
            return False
    ...
    def error_occurred(self, node_id, error_factor = 1):
        """
        ...
        """
        node = self.node_errors.get(node_id)
        node['errors'] = node.get('errors', 0) + error_factor
    ...
    def update(self, job):
        ...
        #MODIFIED LightSync
            ...
            # self.suffix_hash += hashed
            # self.logger.update_stats('suffix.hashes', hashed)"""
            job_nodes = [n for n in job['nodes'] if not self.error_limited(n['id'])]
            attempts_left = len(job_nodes)
            handle_nodes = [n for n in self.object_ring.get_more_nodes(int(job['partition'])) if not self.error_limited(n['id'])]

            nodes = itertools.chain(job_nodes, handle_nodes)
            while attempts_left > 0:
                ...
                ##
                    ...
                    if isinstance(e, eventlet.Timeout):
                        error_factor = int(1.0 * self.error_suppression_limit * self.error_suppression_interval/ e.seconds)
                    else:
                        error_factor = 1
                    self.error_occurred(node['id'], error_factor=error_factor)
...
##
    ...
    def collect_jobs(self):
        ...
        ## M
        now = time.time()
        # sorted by failed time
        nodes_order = sorted([dev['id'] for dev in self.object_ring.devs],
                             key=lambda id: self.node_errors[id]['last_error']
                             if 'last_error' in self.node_errors[id] and self.last_replication_time[0] <
                                                                         self.node_errors[id]['last_error'] <
                                                                         self.last_replication_time[1]
                             else now)

        failed_jobs = dict([(k,[]) for k in nodes_order])
        #
        ...
        ##
                    if nodes[0]['id'] in failed_jobs:
                        failed_jobs[nodes[0]['id']].append(
                            dict(path=job_path,
        ...
        try:
            all_jobs = []
            for failed_job in [failed_jobs[j] for j in nodes_order]:
                all_jobs.extend(failed_job)
            all_jobs.extend(jobs)
            jobs = all_jobs

            # if len(nodes_order) > 0:  # if errors
            #     jobs.sort(key=lambda job: nodes_order.index(job["nodes"][0]['id']))
        except ValueError:
            pass
        #
    ...
    def run_forever(self, *args, **kwargs):
        ...
        while True:
            ...
            total = (time.time() - start) / 60
            ## M
            self.last_replication_time = (start, time.time())
            #
            self.logger.info(
